...
Given an Excel .xls spreadsheet with the required columns, gmail account credentials, and a text file with Python-formatted parameter placeholders ({firstName} replaces the curly brackets {} and the name inside with the value of the spreadsheet cell of the column with header "firstName" and the current row being used to send an email), will send an email to each sendto email address row (one email per row).
